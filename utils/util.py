...
def save_csv_preds(preds: List[Tuple[str,str,str]], save_dir, file_name='predictions.csv'):
    """Save predictions `preds` to a CSV file named `file_name` in `save_dir`.
    ...
    save_path = os.path.join(save_dir, file_name)
    import pandas as pd
    df = pd.DataFrame(preds,columns=["id","text","res"])
    df.to_csv(save_path)
...
def pred_to_csv(id_list,pred_list,target_list):
    assert len(id_list) == len(pred_list)
    import pandas as pd
    df = pd.DataFrame({'id_list': id_list, 'pred_list': pred_list, 'target_list': target_list})
    df.to_csv('./outputs/stage2/box_preds.csv',index=False)
